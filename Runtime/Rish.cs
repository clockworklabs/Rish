...
namespace RishUI
{
    public static partial class Rish
    {
        ...
        public static ManagedContext GetOwnerContext<T1, T2>(T1 reference) where T1 : unmanaged, IReference<T2> where T2 : class, IManaged => GetPool<T2>()?.GetWrapper<T2>(reference.ID)?.OwnerContext;
    }
}